function combine_sen_img_mod_plane_eff(senimg_name, num_beds, rings_per_bed, bedStartRing, overlap_percent, max_MUD)
...
out_name = [out_name, 'combine_', num2str(num_beds), 'beds_', num2str(rings_per_bed), 'rings_from', num2str(bedStartRing), '_', num2str(overlap_percent), '%overlap.from_mod_plane_eff', '.sen_img'];

for MUD = 0 : max_MUD
    senimg_name_temp = senimg_name(1:strfind(senimg_name, '.sen_img'));
    senimg_name_temp = [senimg_name_temp, num2str(num_beds), 'beds_', num2str(rings_per_bed), 'rings_from', num2str(bedStartRing), '_', num2str(overlap_percent), '%overlap_MUD', num2str(MUD), '.sen_img']
...
if rem(final_ring-num_gaps_end,84) == 0     
    final_ring = final_ring-1       %because last ring cannot be a gap
end
